...
Throughout the supplement, `motivation/engagement` is used as the observational counterpart of the theoretical `preference field`: how worthwhile participation in ordinary life still feels. The corresponding control-related proxy is described as `executive-control` in the MCS analyses and as `cognitive-control` in the ABCD analyses, matching the local measurement framing while retaining the same link to `policy precision`: how consistently action can be selected and carried through once engagement is attempted. 
...
This section tests the same Notebook 03 prediction in the ABCD cohort using the focal age-13 balance specification. Here the balance index is constructed as $\psi = \log(\text{CBCL stress} / \text{Positive affect})$, using `ses-03A` parent-reported CBCL stress (`mh_p_cbcl__strs_sum`) as the distress-related component and the `ses-03A` youth-reported NIH Toolbox Positive Affect Items sum score (`mh_y_pai_sum`) as the protective denominator. The retained binary outcome is transition into probable internalising caseness, operationalised as `ses-05A` CBCL internalising `T >= 64` among adolescents below that threshold at `ses-03A`.

The positive-affect denominator follows the ABCD scoring documentation: `mh_y_pai_sum` is the complete-case sum of nine youth-report items (`mh_y_pai_001` to `mh_y_pai_009`) with `777` and `999` excluded. In the local `ses-03A` data that denominator showed good internal consistency (alpha = `r sprintf('%.2f', abcd_positive_affect_reliability$alpha_std[[1]])`, omega = `r sprintf('%.2f', abcd_positive_affect_reliability$omega_total[[1]])`). The pairing of CBCL stress (a parent-reported measure of the child's distress and stress reactivity) with positive affect (a youth-reported measure of positive emotional state) is not identical to the MCS pairing of Kessler distress with WEMWBS wellbeing, but it preserves the same theoretical structure: the ratio of an avoidance-related observable to a protective or appetitive observable, which the theory identifies as the relevant phase-space coordinate. The fitted models are ordinary logistic `glm` models with the same `ses-03A` demographic and socioeconomic covariates as the main ABCD analysis, and all retained models also adjust for baseline internalising severity to keep the comparison anchored to prior symptom burden. The model additionally restricts the baseline sample to adolescents below the threshold so the focus stays on transition rather than cross-sectional severity alone.
...
In the focal ABCD balance analysis, the pattern is clean and supportive. For later probable internalising caseness, the age-13 balance index performs best of the three single-predictor models, with the lowest AIC and the highest AUC, outperforming both stress alone and positive affect alone. That places the ABCD result in the same qualitative direction as the MCS result: in both cohorts, the log-ratio carries transition-relevant information that neither component captures quite as efficiently on its own.

## Summary

::: {.summary-box}
The ABCD analyses test whether the same ordering of predictor families appears in an independent cohort with different measures. The main models use `ses-02A` predictors and covariates with `ses-05A` outcomes, and with that adjustment set the direct coefficient-comparison test now supports the ordering claim in the main parent-report model. Sleep remains the clearest contextual moderator of that pathway, and the BPM sensitivity analyses provide an additional youth-report check, with clearer support for BPM internalising than for BPM attention. The focal balance-index analysis then shows that the age-13 ratio $\psi = \log(\text{CBCL stress}_{\text{ses-03A}} / \text{Positive affect}_{\text{ses-03A}})$ performs best for later probable internalising caseness by `ses-05A`, supporting the balance-index prediction in ABCD as well as in MCS.
:::
...
**The strongest cross-cohort pattern is convergent in both cohorts.** MCS supports the ordering in which age-14 motivation and engagement are more central than age-14 executive-control for later distress after adjustment for age-14 demographic and socioeconomic covariates, and ABCD now does the same for age-12 cognitive-control after adjustment for baseline demographic, socioeconomic, neighbourhood, and site covariates in the main parent-report model.

A short interpretive point is important for the balance indices. These ratios should not be treated as universal superior predictors of all later outcomes. The current results are better read as evidence that ratio-type measures can carry transition-relevant information when the endpoint is clinically proximate and the numerator and denominator are measured in a defensible developmental window. Their usefulness should therefore be tested rather than assumed in each cohort.
...
::: {.summary-box}
Across MCS and ABCD, the study provides indirect observational support for the narrower claim that motivation/engagement-related processes may be more central than executive-control-related processes in MCS and cognitive-control-related processes in ABCD for persistence- and recovery-relevant developmental pathways. The models make their adjustment sets explicit and retain the same broad conclusion after standard demographic and socioeconomic covariate adjustment. That support remains strongest and most extensive in the survey-weighted MCS analyses, but it is also statistically supported in the main adjusted ABCD model. The balance-index analyses provide a further, theory-derived test: whether log-ratio measures constructed from distress and wellbeing in MCS, and from stress and positive affect in ABCD, carry additional transition-relevant information for later clinically relevant outcomes beyond either component alone.
:::
